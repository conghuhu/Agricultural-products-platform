...
<script>
	...
	export default {
		...
		computed: {
			...
			copyFlowList() {
				return this.cloneData(this.getValue());
			}
		},
		methods: {
			getValue() {
			...
			remove(id) {
				...
				if (index != -1) {
					this.$emit('input', this.getValue().splice(index, 1));
					this.$emit("update:modelValue", this.getValue().splice(index, 1));
				}
			},
			// 修改某条数据的某个属性
			modify(id, key, value) {
				...
				// 修改父组件的数据中的对应id的条目
				index = this.getValue().findIndex(val => val[this.idKey] == id);
				if (index != -1) {
					// 首先复制一份value的数据
					let data = this.cloneData(this.getValue());
